...
export const renderSection = (section, index) => {
  ...
  const parseInlineMarkdown = (text) => {
    ...
    return textWithLinks.map((part, idx) => {
      ...
      while ((boldMatch = boldRegex.exec(part.content)) !== null) {
        ...
        segments.push(<strong key={`bold-${idx}-${boldMatch.index}`} className="font-semibold text-gray-900">{boldMatch[1]}</strong>);
...
export const insertImagesIntoSections = (sections, images) => {
